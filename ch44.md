...
![Figure 44-1: Using a pipe to connect two processes](asset/image/fig44-1)
...
The shell uses pipes to connect filters. (44.4)
...
Instead, UNIX domain stream socket pairs (socketpair(), 57.5) provide a standardized bidirectional communication mechanism equivalent to stream pipes.
:::

### Writes of up to PIPE_BUF bytes are guaranteed to be atomic
SUSv3: PIPE_BUF should be gte _POSIX_PIPE_BUF (512)
...
> Linux2.6.11, the pipe capacity is 65536 bytes.
...
![Figure 44-2: Process file descriptors after creating a pipe](asset/image/fig44-2)

![Figure 44-3: Setting up a pipe to transfer data from a parent to a child](asset/image/fig44-3)
...
Many writers to one reader is useful on a FIFO (44.8).
...
A common scenario is to use pipe for communication between two siblings. Their parent creates the pipeand then creates the two children.
...
![Figure 44-4: Overview of process relationships and pipe usage for popen()](asset/image/fig44-4)

### popen() vs system()
popen() builds the pipe, performs descriptor duplication, closes unused descriptors, and handles all of the details of fork() and exec(), the shell processing is also performed on the command.

The cost is efficiency. At least two processes will be created: shell process and process command executed by the shell.
...
Use open() with O_NONBLOCK is a more standardized method for doing so. (44.9)
...
![Figure 44-5: Using a FIFO and tee(1) to create a dual pipeline](asset/image/fig44-5)
...
### Design
![Figure 44-6: Using FIFOs in a single-server, multiple-client application](asset/image/fig44-6)

* server (well-known name): /tmp/seqnum_sv
* create FIFO in /tmp
* sender?
    * each client creates FIFO and send the pathname to server
    * agree on the convention for FIFO pathname :point_left: 
...
* message boundary?
    ...
    * fixed-length messages :point_left: 
        simple to program
        cons: channel capacity is wasted (padded). message can be cutoff if greater than the length specifed
        
![Figure 44-7: Separating messages in a byte stream](asset/image/fig44-7)
...
![Figure 44-8: Deadlock between processes opening two FIFOs](asset/image/fig44-8)
